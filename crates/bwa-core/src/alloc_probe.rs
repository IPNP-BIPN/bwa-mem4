...
/// Every bucket's label, and by its index the bucket itself.
///
/// One flat namespace shared by three crates, which is why it lives in `bwa-core` rather than next
/// to either user. The first nine mirror `bwa-mem4`'s `stage_time::Stage` discriminants exactly, so
/// the wall-clock table and this one can be read side by side (a unit test in the binary crate
/// checks the two name lists still match). Then the two pipeline threads and a catch-all, and last
/// the sub-phases of `align`, which `bwa-mem4-mem` tags from inside [`crate`]'s dependents.
///
/// A sub-phase tag REPLACES the enclosing `align` tag rather than nesting under it, so the rows are
/// disjoint and `align` keeps only what none of its sub-phases claimed.
...
/// Number of pipeline stages, i.e. how many leading entries of [`BUCKET_NAMES`] are owned by the
/// binary crate's `stage_time::Stage`.
pub const N_STAGES: usize = 9;
/// Bucket for allocations made by the FASTQ reader thread.
pub const B_READER: u8 = 9;
/// Bucket for allocations made by the SAM/BAM writer thread.
pub const B_WRITER: u8 = 10;
/// Bucket for everything outside a named stage: index load, header emission, teardown, and any
/// thread that is neither the reader, the writer, nor inside a tagged scope.
pub const B_UNSTAGED: u8 = 11;
/// Round-1 SMEM collection for a whole read chunk (`mem_collect_smem_batched`).
pub const B_ALIGN_SEED: u8 = 12;
/// Suffix-array resolution: the flat position list and the lockstep `get_sa_batch`.
pub const B_ALIGN_SA: u8 = 13;
/// Chain construction and filtering (`build_chains_from_resolved`, `mem_chain_flt`).
pub const B_ALIGN_CHAIN: u8 = 14;
/// Banded extension: job collection, the DP kernels, and the discard/compaction passes.
pub const B_ALIGN_EXTEND: u8 = 15;
...
/// Arm the probe. Called by the binary crate once it has read `BWA4_STAGE_ALLOC` and confirmed the
/// counting allocator is actually installed.
///
/// Call it as early in `main` as possible: allocations made before this point are not counted,
/// which for the `mem` path means clap's command line parsing and nothing else. The environment is
/// read THERE and not here, on a thread where allocating is safe, which is the whole reason the
/// flag is a plain atomic rather than a `OnceLock` consulted lazily from inside the allocator.
///
/// # Parameters
///
/// - `serialize`: whether the caller's pipeline must run one batch at a time, which is what makes
///   the per-stage split exact. See [`serialize_batches`].
pub fn arm(serialize: bool) {
    ENABLED.store(true, Relaxed);
    SERIALIZE.store(serialize, Relaxed);
}
...
/// Credit every subsequent allocation to `bucket`, until the returned guard drops.
///
...
/// # Parameters
///
/// - `bucket`: the bucket the calling thread is entering, one of the `B_*` constants or a
///   `stage_time::Stage` discriminant cast to `u8`.
///
/// # Returns
///
...
#[inline]
pub fn enter(bucket: u8) -> StageGuard {
    if !enabled() {
        return StageGuard {
            prev: B_UNSTAGED,
            active: false,
    ...
    StageGuard {
        prev: STAGE.swap(bucket, Relaxed),
        active: true,
    }
}
...
mod tests {
    ...
    /// The `align` stage's own bucket, i.e. `stage_time::Stage::Align`'s discriminant. Spelled out
    /// here because this crate does not know that enum.
    const B_ALIGN: u8 = 2;

    /// The table and its length must not drift apart, and the named constants must point at the
    /// labels they claim: an off-by-one here would silently retitle every row.
    #[test]
    fn bucket_constants_point_at_their_labels() {
        assert_eq!(BUCKET_NAMES.len(), N_BUCKETS);
        assert_eq!(BUCKET_NAMES[B_READER as usize], "reader");
        assert_eq!(BUCKET_NAMES[B_WRITER as usize], "writer");
        assert_eq!(BUCKET_NAMES[B_UNSTAGED as usize], "unstaged");
        assert_eq!(BUCKET_NAMES[B_ALIGN_SEED as usize], "align:seed");
        assert_eq!(BUCKET_NAMES[B_ALIGN_SA as usize], "align:sa");
        assert_eq!(BUCKET_NAMES[B_ALIGN_CHAIN as usize], "align:chain");
        assert_eq!(BUCKET_NAMES[B_ALIGN_EXTEND as usize], "align:extend");
    }
    ...
    fn allocations_land_in_the_current_stage_and_the_peak_is_the_live_maximum() {
        ...
        {
            let _tag = enter(B_ALIGN);
            record_alloc(500);
            assert_eq!(BYTES[B_ALIGN as usize].load(Relaxed), 500);
            // Live is 1500 here, and that is the run's high-water mark.
            assert_eq!(PEAK.load(Relaxed), 1500);
            assert_eq!(PEAK_BUCKET.load(Relaxed), B_ALIGN);
        }
